...
VkShaderModule RenderPipeline::createShaderModule(std::vector<char> shader)
{
...
VkRenderPass RenderPipeline::createRenderPass(VkFormat colorFormat)
{
...
RenderPipeline::RenderPipeline(Device& device, const char* vertPath, const char* fragPath, VkExtent2D extents, VkFormat colorFormat)
    :m_device(device)
{
    ...
    m_renderPass = createRenderPass(colorFormat);
    ...
    if (vkCreateGraphicsPipelines(m_device.m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_pipeline) != VK_SUCCESS) {
